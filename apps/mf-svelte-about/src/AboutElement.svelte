<script>
  ...
  function normalizeData() {
    ...
    skills.forEach(skill => {
      const category = pickLocale(skill.category) || 'Other';
      if (!grouped[category]) {
        grouped[category] = [];
      }
...
<div class="px-4 py-8 md:py-12">
  ...
  {#if profile && !loading && !error}
      ...
      <div class="relative z-10 max-w-4xl mx-auto text-center">
        ...
        <div class="flex flex-wrap justify-center gap-4 mb-8">
          <a href="https://www.linkedin.com/in/samet-yilmazcelik" target="_blank" rel="noopener noreferrer" 
             class="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-lg font-medium transition-all duration-200 hover:scale-105 shadow-lg shadow-accent/20 bg-accent hover:bg-accent/90 text-white">
        ...
        <div class="relative border-l border-border/30 ml-3 md:ml-6 space-y-12 pb-12">
          {#each experiencesNormalized as exp (exp.id || exp)}
            <div class="relative pl-8 md:pl-12 group">
              <!-- Timeline Dot -->
              <div class="absolute -left-[5px] md:-left-[5px] top-6 w-3 h-3 rounded-full bg-accent ring-4 ring-background shadow-[0_0_15px_rgba(20,184,166,0.5)] group-hover:scale-125 transition-transform duration-300"></div>
                    ...
                    <ul class="space-y-3">
                      {#each exp.bulletsText as bullet}
                        <li class="text-text-secondary flex items-start gap-3 text-base leading-relaxed">
                          <span class="text-accent mt-2 w-1.5 h-1.5 rounded-full bg-accent/80 flex-shrink-0 shadow-[0_0_8px_rgba(20,184,166,0.4)]"></span>
                          <span>{bullet}</span>
                        </li>
                      {/each}
                    </ul>
                ...
                <div class="flex flex-wrap gap-2">
                  {#each category.items as item}
                    <div class="group/item relative inline-flex items-center gap-2 px-3 py-1.5 bg-surface-elevated/30 border border-border/30 rounded-lg hover:bg-surface-elevated hover:border-accent/40 transition-all duration-300">
                      ...
                      {#if item.level}
                         <span class="w-1.5 h-1.5 rounded-full {item.level === 'Advanced' || item.level === 'İleri' ? 'bg-emerald-400' : item.level === 'Intermediate' || item.level === 'Orta' ? 'bg-yellow-400' : 'bg-blue-400'}"></span>
                      {/if}
                    </div>
                  {/each}
                </div>
    ...
    {#if !experiencesNormalized || experiencesNormalized.length === 0}
      <div class="mt-16 pt-8 border-t border-border/30">
        <p class="text-center text-text-muted">{locale === 'tr' ? 'Deneyim verisi bulunamadı' : 'No experience data found'}</p>
      </div>
    {/if}

    {#if !skillsGrouped || skillsGrouped.length === 0}
      <div class="mt-16 pt-8 border-t border-border/30">
        <p class="text-center text-text-muted">{locale === 'tr' ? 'Yetenek verisi bulunamadı' : 'No skills data found'}</p>
      </div>
    {/if}
  {/if}
</div>
